...
public class ComparisonPredicateTests {
  ...
  public void testFilterWithComparisonPredicates(String type, String field, String value, boolean expected) {
    ...
    for (Map<String, String> user : users) {
      assertEquals(expected, filter.matches(user),
          "Expected user to " + (expected ? "" : "not ") + "match the filter: " + type
              + " " + field + " "
              + value);
    }
  }
  ...
  @DisplayName("Test ComparisonPredictate parseFromString function - Multiple Users")
  public void testComparisonPredicateParseFromString_v1(String type, String field, String value,
      boolean expected) {
    Filter filter = new Filter(new ComparisonPredicate(type, field, value));
  ...
  @DisplayName("Test ComparisonPredictate parseFromString function - Single Users")
  public void testComparisonPredicateParseFromString_v2(String type, String field, String value, boolean expected) {
    Filter filter = new Filter(new ComparisonPredicate(type, field, value));
    ...
    assertEquals(expected, parsedFilter.matches(user),
        "Expected user to " + (expected ? "" : "not ") + "match the filter: " + type + " "
            + field
            + " "
            + value);
